...
/** @defgroup LCD_LOG_Private_TypesDefinitions
* @{
*/ 
/**
* @}
...
PUTCHAR_PROTOTYPE
{
  ...
  if(LCD_Lock == DISABLE)
  {
    ...
    {
      LCD_CacheBuffer[LCD_CacheBuffer_yptr_bottom].line[LCD_CacheBuffer_xptr++] = ch;
    }   
    else 
    {
...
static void LCD_LOG_UpdateDisplay (void)
{
     ...
     (LCD_CacheBuffer_yptr_bottom  >= LCD_CacheBuffer_yptr_top))
  {
    ...
    LCD_DisplayStringLine ((YWINDOW_MIN + LCD_CacheBuffer_yptr_bottom) * cFont->Height,
                           LCD_CacheBuffer[cnt + LCD_CacheBuffer_yptr_bottom].line); 
  }
  else
  {
